...
export class RoleService {
  ...
  public static readonly active = (id: string): Promise<AxiosResponse<RoleVM>> => {
    return useHttp().put<RoleVM>(`${environment.apiEndpont}${environment.api['basic-api']['role'].active}${id}`);
  }

  public static readonly deactive = (id: string): Promise<AxiosResponse<RoleVM>> => {
    return useHttp().put<RoleVM>(`${environment.apiEndpont}${environment.api['basic-api']['role'].deactive}${id}`);
  }
}