...
import { AccountCanvasComponent } from '..';

export interface IAccountControlComponentProps extends IBaseProps {
  input: {
...
export const AccountControlComponent: React.FC<IAccountControlComponentProps> = (props: IAccountControlComponentProps) => {
  const region = useSelector<RootState, 'vi' | 'en' | 'jp'>((state) => state.language.language.region);
  const config = environment.i18n.data.core.modules.account.components['account-control'][region];
      ...
      <div style={{ width: '100%', border: '1px dashed gray', marginBottom: 5, background: 'transparent' }}>
        <span style={{ display: 'block', textAlign: 'right', padding: 10, background: 'black' }}>
          ...
          <Popover placement="bottomRight" trigger="click" content={
            <>
              <AccountCanvasComponent input={{ control: props.input.item }} output={{ edit: props.output.edit }} />
            </>
          }>
        ...
        <span style={{ display: 'block', padding: 10, minHeight: 55 }}>
          {renderInput(props.input.item as AccountExtraInformationCM | AccountExtraInformationUM)}
        </span>
      </div>
    </Col>
  );
