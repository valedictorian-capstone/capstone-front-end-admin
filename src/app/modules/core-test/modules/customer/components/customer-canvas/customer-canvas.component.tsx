...
import { CustomerOptionComponent } from '..';
import './customer-canvas.component.css';

export interface ICustomerCanvasComponentProps extends IBaseProps {
  input: {
    control: CustomerExtraInformationCM | CustomerExtraInformationUM,
  };
  output: {
...
export const CustomerCanvasComponent: React.FC<ICustomerCanvasComponentProps> = (props: ICustomerCanvasComponentProps) => {
  const region = useSelector<RootState, 'vi' | 'en' | 'jp'>((state) => state.language.language.region);
  const config = environment.i18n.data.core.modules.customer.components['customer-canvas'][region];
  const [control, setControl] = React.useState<CustomerExtraInformationCM | CustomerExtraInformationUM>({
    ...props.input.control,
        ...
        {(control.type === 'select' || control.type === 'multi-select' || control.type === 'check-box-group' || control.type === 'radio' || control.type === 'auto-complete') && (
          <>
            ...
            {(control.options as any).map((option: { value: string, label: string }, i: number) => (
              <CustomerOptionComponent key={uuid()} input={{ option }} output={{
                save: (op) => {
