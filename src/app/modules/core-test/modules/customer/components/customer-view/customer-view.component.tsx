...
import { CustomerControlComponent, CustomerExtraComponent, CustomerPaletteComponent } from '..';
import './customer-view.component.css';
export interface ICustomerViewComponentProps extends IBaseProps {
  input: {
  };
  output: {
    back: () => void,
...
export const CustomerViewComponent: React.FC<ICustomerViewComponentProps> = (props: ICustomerViewComponentProps) => {
  const region = useSelector<RootState, 'vi' | 'en' | 'jp'>((state) => state.language.language.region);
  const config = environment.i18n.data.core.modules.customer.components['customer-view'][region];
  const dispatch = useDispatch();
  const arr = useSelector<RootState, CustomerExtraInformationVM[]>((state) => state.customerExtraInformation.arr);
  const [controls, setControls] = React.useState<CustomerExtraInformationCM[] | CustomerExtraInformationUM[]>(arr.sort((a, b) => a.position - b.position));
  const [, drop] = useDrop({
  ...
  return (
    <Row gutter={24} style={{ height: (window.innerHeight - 140) }}>
      ...
      >
        <CustomerPaletteComponent input={{}} output={{}} />
      </Col>
      ...
      <Col className="gutter-row" span={6}>
        <div style={{ height: window.innerHeight - 140, border: '1px solid gray' }}>
          {arr.map((item, index) => (
            <CustomerExtraComponent key={index} input={{ item, index, disabled: false }} output={{}} />
          ))}
        </div>
      </Col>
    </Row>
