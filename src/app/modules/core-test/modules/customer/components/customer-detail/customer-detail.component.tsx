...
import { CustomerExtraInformationVM, CustomerUM, CustomerVM } from '@view-models';
...
export const CustomerDetailComponent: React.FC<ICustomerDetailComponentProps> = (props: ICustomerDetailComponentProps) => {
  ...
  React.useEffect(() => {
    ...
    for (const extra of props.input.customerExtraInformations) {
        if (!extra.isDelete) {
          const valueExtra = props.input.row?.customerExtraInformationDatas.find((customerExtraInformationData) => customerExtraInformationData.customerExtraInformation.id === extra.id);
          let value: any = '';
          if (valueExtra) {
            switch (valueExtra.customerExtraInformation.type) {
              case 'date-picker':
