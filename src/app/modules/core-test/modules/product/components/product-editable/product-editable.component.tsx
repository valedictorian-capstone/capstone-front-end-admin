...
import './product-editable.component.css';

export interface IProductEditableComponentProps extends IBaseProps {
  input: {
...
export const ProductEditableComponent: React.FC<IProductEditableComponentProps> = (props: IProductEditableComponentProps) => {
  ...
  return (
    <div className="ant-form ant-form-horizontal product-editable" style={{ width: '100%', height: '100%' }}>
      <Row ref={drop} style={{ width: '100%', height: extraInformations.length > 3 ? ((extraInformations.length * 30) + 100 + '%') : '100%', alignContent: 'baseline' }}>
        {extraInformations.map((item, index) => (
          <FormControlSettingComponent key={item.id + '-editable'} input={{ index, item, canEdit: active === index, active, isUseDeactive: true }} output={{
            ...
            onRemove: () => {
              if (item.isDelete) {
                setProductExtraInformations(extraInformations.map((formControl) => formControl.id !== item.id ? formControl : { ...formControl, isDelete: !formControl.isDelete }));
              } else {
                setProductExtraInformations(extraInformations.filter((formControl) => formControl.id !== item.id));
              }             },
            onDone: (data) => {
              console.log(data);
              extraInformations[index] = data as ExtraInformationVM;
              setProductExtraInformations([...extraInformations]);
              setActive(-1);
            },
          }} />
        ))}
