...
import './group-detail.component.css';
export interface IGroupDetailComponentProps extends IBaseProps {
  input: {
    action: 'create' | 'edit' | '',
    row: GroupVM | undefined,
  };
  output: {
    onDone: () => void,
  };
...
export const GroupDetailComponent: React.FC<IGroupDetailComponentProps> = (props: IGroupDetailComponentProps) => {
  const region = useSelector<RootState, 'vi' | 'en' | 'jp'>((state) => state.language.language.region);
  const [submit, setSubmit] = React.useState<'completed' | 'processing' | 'cancel' | 'none'>('none');
  const [form] = Form.useForm<GroupUM>();
  ...
  return (
    <div id={props.input.row?.id} key="new-group" className="group-item selected" style={{ display: props.input.action === 'edit' && props.input.row ? 'block' : 'none' }}>
            ...
            <div className="group-item-footer-content-middle-buttons">
              <Button size="middle" shape="circle" icon={<CheckOutlined />} style={{ marginRight: 10 }} onClick={() => {
                ...
                form.validateFields().then(() => {
                  setTimeout(() => {
                    dispatch(useGroupAction().update({ ...props.input.row, ...form.getFieldsValue() },
                      () => {
