import { CheckCircleOutlined, DownloadOutlined, EditOutlined, FileExcelOutlined, MinusCircleOutlined, PlusCircleFilled, PlusOutlined, SettingOutlined, SortAscendingOutlined, SortDescendingOutlined } from '@ant-design/icons';
...
export const AccountListComponent: React.FC<IAccountListComponentProps> = (props: IAccountListComponentProps) => {
  ...
  const config = environment.i18n.data.core.modules.account.components['account-list'][region];
  const arr = useSelector<RootState, AccountVM[]>((state) => state.account.arr);
  const dispatch = useDispatch();
  const [visible, setVisible] = React.useState<boolean>(false);
  const [selected, setSelected] = React.useState<AccountVM>();
  ...
  const [rowFilter, setRowFilter] = React.useState<AccountVM[]>(arr.sort());
  ...
  return (
    <List
      ...
      header={
        <>
          <AccountCreateComponent input={{
            ...
            onCreate: (form) => {
              dispatch(useAccountAction().create({ ...form }));
            },
            onClose: (() => setVisible(false)),
          }} />
          <AccountDetailComponent input={{
            data: selected,
          }} output={{
            onUpdate: (form) => {
              dispatch(useAccountAction().update({ ...form }));
            },
                ...
                {ids.length > 0 && (
                  <>
                    <Popconfirm
                      ...
                      placement="top"
                      onConfirm={() => {
                        dispatch(useAccountAction().active(ids));
                        setIds([]);
                      }}
                    ...
                    <Popconfirm
                      ...
                      placement="top"
                      onConfirm={() => {
                        dispatch(useAccountAction().deactive(ids));
                        setIds([]);
                      }}
      ...
      renderItem={(item: AccountVM, i) => (i >= (((pageIndex - 1) * itemSize)) && i <= (pageIndex * itemSize) - 1) && (
                ...
                <Popover placement="left" trigger="hover" content={
                  <>
                    ...
                    <Popconfirm
                      ...
                      placement="top"
                      onConfirm={() => {
                        dispatch(useAccountAction().active([item.id]));
                      }}
                    ...
                    <Popconfirm
                      ...
                      placement="top"
                      onConfirm={() => {
                        dispatch(useAccountAction().deactive([item.id]));
                      }}
