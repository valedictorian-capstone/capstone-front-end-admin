...
import { AccountOptionComponent } from '..';
import './account-canvas.component.css';

export interface IAccountCanvasComponentProps extends IBaseProps {
  input: {
    control: AccountExtraInformationCM | AccountExtraInformationUM,
  };
  output: {
    edit: (control: AccountExtraInformationCM | AccountExtraInformationUM) => void,
  };
}

export const AccountCanvasComponent: React.FC<IAccountCanvasComponentProps> = (props: IAccountCanvasComponentProps) => {
  const region = useSelector<RootState, 'vi' | 'en' | 'jp'>((state) => state.language.language.region);
  const config = environment.i18n.data.core.modules.account.components['account-canvas'][region];
  const [control, setControl] = React.useState<AccountExtraInformationCM | AccountExtraInformationUM>({
    ...props.input.control,
        ...
        {(control.type === 'select' || control.type === 'multi-select' || control.type === 'check-box-group' || control.type === 'radio' || control.type === 'auto-complete') && (
          <>
            ...
            {(control.options as any).map((option: { value: string, label: string }, i: number) => (
              <AccountOptionComponent key={uuid()} input={{ option }} output={{
                save: (op) => {
