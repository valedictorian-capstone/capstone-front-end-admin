...
import { AccountVM, DepartmentVM, RoleVM } from '@view-models';
import { Button, Col, Input, Popover, Row } from 'antd';
import React from 'react';
import { AccountHelperComponent } from '..';
import './account-support.component.css';

export interface IAccountSupportComponentProps extends IBaseProps {
  input: {
  ...
  output: {
    onCreate: () => void,
    onSearch: (rows: AccountVM[]) => void,
    onSetting: () => void,
  };
}

export const AccountSupportComponent: React.FC<IAccountSupportComponentProps> = (props: IAccountSupportComponentProps) => {
  ...
  return (
    <div className="account-support">
      <Row>
        <Col xs={10} sm={10} md={12} lg={12} xl={16} xxl={16} className="account-support-left">
          <div>
            ...
            {props.input.action === 'setting' && <Popover id="rop-test" placement="topLeft" trigger="click" content={<AccountHelperComponent input={{ horizontal: true }} output={{}} />}>
        ...
        <Col xs={14} sm={14} md={12} lg={12} xl={8} xxl={8} className="account-support-right">
          <div>
